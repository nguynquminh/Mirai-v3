module.exports = function({
    ...
    return async function({
        ...
        if (event.isGroup == !![])
            ...
            var permssion = 0;
            try {
                // Lấy dữ liệu thread với kiểm tra null
                const threadData = await Threads.getData(threadID);
                
                // Kiểm tra tồn tại threadInfo và adminIDs
                if (threadData && threadData.threadInfo) {
                    const threadInfoo = threadData.threadInfo;
                    
                    // Sử dụng optional chaining và nullish coalescing để xử lý trường hợp không có adminIDs
                    const adminIDs = threadInfoo.adminIDs ?? [];
                    
                    // Kiểm tra quyền admin
                    const isThreadAdmin = adminIDs.some(el => el.id === senderID);
                    
                    // Xác định mức độ permission
                    if (ADMINBOT.includes(senderID.toString())) {
                        permssion = 2;
                    } else if (NDH.includes(senderID.toString())) {
                        permssion = 3;
                    } else if (isThreadAdmin) {
                        permssion = 1;
                    }
                } else {
                    console.warn(`Thread data not found for threadID: ${threadID}`);
                }
            } catch (error) {
                console.error(`Error checking permissions for thread ${threadID}:`, error);
                // Mặc định giữ permission = 0 nếu có lỗi
            }
            
            // Giữ nguyên phần rolePermissions và kiểm tra tiếp theo
